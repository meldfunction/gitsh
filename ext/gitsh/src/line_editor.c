...
{
    return rb_funcall(readline, rb_intern("completion_append_character="), 1,
        append_chr);
}
...
{
    return rb_funcall(readline, rb_intern("completion_proc="), 1, proc);
}

VALUE
line_editor_set_input(VALUE module, VALUE input)
{
    return rb_funcall(readline, rb_intern("input="), 1, input);
}

VALUE
line_editor_set_output(VALUE module, VALUE output)
{
    return rb_funcall(readline, rb_intern("output="), 1, output);
}

VALUE
line_editor_readline(VALUE module, VALUE prompt, VALUE add_to_history)
{
    return rb_funcall(readline, rb_intern("readline"), 2,
...
{
    return rb_funcall(readline, rb_intern("line_buffer"), 0);
}

VALUE
line_editor_emacs_editing_mode(VALUE module)
{
    return rb_funcall(readline, rb_intern("emacs_editing_mode"), 0);
}
